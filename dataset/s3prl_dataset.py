...
class SequenceDataset(Dataset):
    ...
    def __init__(self, split, bucket_size, text_encoder, tokenizer, dictionary, libri_root, bucket_file, **kwargs):
        ...
        # Transcripts
        Y, self.Y_text = self._load_transcript(X)
    ...
    def _load_transcript(self, x_list):
        ...
        def process_trans(transcript):
            ...
            return transcript, " ".join(list(transcript.replace(" ", "|"))) + " |"
        ...
        for dir in split_spkr_chap_list:
            ...
            with open(path, "r") as trans_f:
                for line in trans_f:
                    ...
                    text, labels = process_trans(" ".join(lst[1:]))
                    trsp_sequences[lst[0]] = labels
                    texts[lst[0]] = text

        return trsp_sequences, texts
    ...
    def __getitem__(self, index):
        ...
        wav_embedding_batch = pad_sequence(wav_batch).transpose(0, 1)
        ...
        text_batch = [self.Y_text[self._parse_x_name(x_file)] for x_file in self.X[index]]
        text_embedding_batch = [self.encode_text(val) for val in text_batch]
        text_embedding_batch = pad_sequence(text_embedding_batch).transpose(0, 1)
        return wav_embedding_batch, label_batch, filename_batch, text_batch, text_embedding_batch # bucketing, return ((wavs, labels))

    def collate_fn(self, items):
        assert len(items) == 1
        return items[0][0], items[0][1], items[0][2], items[0][3], items[0][4] # hack bucketing, return (wavs, labels, filenames)
...
#
# dataset = SequenceDataset(split='test-clean', dictionary=Dictionary(), text_encoder=text_encoder, tokenizer=tokenizer,
